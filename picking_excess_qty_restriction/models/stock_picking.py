    ...
    def button_validate(self):
        ...
        if delivery_qty:
            for line in (self.move_ids_without_package.filtered
                (lambda l: l.quantity_done > l.product_uom_qty and self.picking_type_id.code in ['outgoing','incoming','internal','mrp_operation'] or l.sale_line_id)):
                    raise ValidationError(
                        _("Product level does not allow for more quantity to be delivered - '%s' Demand - '%s' Done - '%s'")
                        % (line.product_id.name,line.product_uom_qty,line.quantity_done))
