...
class SaleOrder(models.Model):
    ...
    origin = fields.Char(string='Order Ref No',
    ...
    def _compute_do_reference(self):
        ...
        for line in self:
            move = self.env['stock.move'].search(
                [('sale_line_id', '=', line.id), ('product_id', '=', line.product_id.id)]).filtered(
                lambda l: l.picking_id.state not in ['done', 'cancel']).sorted(lambda line: line.picking_id.id)
            if len(move) == 1:
